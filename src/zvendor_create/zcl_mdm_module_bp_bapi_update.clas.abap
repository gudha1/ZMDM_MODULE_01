CLASS zcl_mdm_module_bp_bapi_update DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_MDM_MODULE_BP_BAPI_UPDATE IMPLEMENTATION.
...
WHERE Supplier =  @GENRALDATA-Businesspartner INTO @DATA(VENDORIDADD).
...
IF VENDORDETAIL-A-BusinessPartner <> '' .
 ...
 DATA(comVEN2) = zmdm_module_vendor_api_update=>GETDATA( VENDOR = VENDORDETAIL-A-BusinessPartner taxkid = taxkid typ = 'COM' ).
 daTA(purVEN2) = zmdm_module_vendor_api_update=>GETDATA( VENDOR = VENDORDETAIL-A-BusinessPartner taxkid = taxkid typ = 'PUR' ).
